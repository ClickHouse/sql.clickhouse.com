...
find . -maxdepth 1 -name '*.json.gz' | xargs -P${NUM_CORES} -I{} bash -c "
gzip -cd {} | jq -c '
[
...
]' | clickhouse-client --input_format_null_as_default 1 --date_time_input_format best_effort --host ${CLICKHOUSE_HOST} --secure --password ${CLICKHOUSE_PASSWORD} --user ${CLICKHOUSE_USER} --query 'INSERT INTO ${TABLE_NAME} FORMAT JSONCompactEachRow' || echo 'File {} has issues'
" && gcloud storage cp *.json.gz ${BUCKET_PATH} && rm *.json.gz
